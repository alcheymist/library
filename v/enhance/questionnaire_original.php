...
abstract class table extends \schema{
    ...
    //stream
    abstract function read()/*:Ibarrell|false*/;
    //
   ...
   //
    function __construct(string $tname){
        //
        $this->name = $tname;
        //
    ...
    private function write_body(array $artefacts){
        ...
        foreach($this->read() as $Ibarrel){
            ...
            //
            //Log the barrel as you carry out the save
            $barrel->save($row);
            //
...
class fuel extends table{
    ...
    //Fetch a row of data
    function read()/*false|Array<basic_value>*/{
        //
        //Stop the fetch if no data is available
        if (!isset($this->data[$this->rowIndex])) return false;
        //
        //Get the array of values
        $values = $this->data[$this->rowIndex];
        //
        return $values; 
    }
...
class csv extends table{
    ...
    //
    //Fetch a row of data
    function read()/*false|Ibarel*/{
        //
        //Get an unlimited line length of data and parse it using CSV format
        $row = \fgetcsv($this->stream, 0, $this->delimiter);
        //
        //At the end of a file, the row is set to false.
        return $row;
    }
    //
    //
...
class query extends table{
    ...
    //
    function __construct(string $tname, string $sql, string $dbname) {
        //
        $this->sql= $sql;
        $this->dbname = $dbname;
        //
    ...
    function read(){
        //
        //Fetch the data using a numeric index
...
class select extends statement{
    ...
    function handle_success(\PDOStatement $stmt ):ans{
       ...
       if  (count($result)>1) throw new Exception("Multiple values in a select statement");
       //
